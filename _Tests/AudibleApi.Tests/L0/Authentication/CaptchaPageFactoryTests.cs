    ...
    public class CreateResultAsync
    {
        ...
        public async Task no_password_throws()
        {
            var body
                ...
                + "<input name='password' />"
                + "<input name='use_image_captcha' />";
            await Assert.ThrowsExceptionAsync<ArgumentException>(() => ResultFactory.CaptchaPage.CreateResultAsync(ApiClientMock.GetClient(), StaticSystemDateTime.Past, new HttpResponseMessage { Content = new StringContent(body) }, new Dictionary<string, string>()));
        }
        ...
        public async Task valid_returns_CaptchaPage()
        {
            ...
            var captchaPage = await ResultFactory.CaptchaPage.CreateResultAsync(ApiClientMock.GetClient(response), StaticSystemDateTime.Past, r, new Dictionary<string, string> { ["password"] = "abc" }) as CaptchaPage;
