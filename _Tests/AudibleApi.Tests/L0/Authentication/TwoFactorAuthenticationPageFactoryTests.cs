namespace Authentic.ResultFactoryTests.TwoFactorAuthenticationPageFactoryTests
{
    ...
    public class IsMatchAsync
    {
        ...
        public async Task null_response_returns_false()
            => (await ResultFactory.TwoFactorAuthenticationPage.IsMatchAsync(null)).ShouldBeFalse();

        [TestMethod]
        public async Task null_content_returns_false()
        {
            var response = new HttpResponseMessage();
            (await ResultFactory.TwoFactorAuthenticationPage.IsMatchAsync(response)).ShouldBeFalse();
        }
        ...
        public async Task is_a_match()
        {
            ...
            (await ResultFactory.TwoFactorAuthenticationPage.IsMatchAsync(response)).ShouldBeTrue();
        }
    }
}
