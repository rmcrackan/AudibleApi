...
namespace Authentic.TwoFactorAuthenticationPageTests
{
    ...
    public class SubmitAsync
    {
        ...
        public async Task null_param_throws()
            => await Assert.ThrowsExceptionAsync<ArgumentNullException>(() => new TwoFactorAuthenticationPage(ApiClientMock.GetClient(), StaticSystemDateTime.Past, "x").SubmitAsync(null));

        [TestMethod]
        public async Task blank_param_throws()
        {
            await Assert.ThrowsExceptionAsync<ArgumentException>(() => new TwoFactorAuthenticationPage(ApiClientMock.GetClient(), StaticSystemDateTime.Past, "body").SubmitAsync(""));
            await Assert.ThrowsExceptionAsync<ArgumentException>(() => new TwoFactorAuthenticationPage(ApiClientMock.GetClient(), StaticSystemDateTime.Past, "body").SubmitAsync("   "));
        }
        ...
        public async Task valid_param_calls_GetResultsPageAsync()
        {
			...
			var page = new TwoFactorAuthenticationPage(ApiClientMock.GetClient(responseToCaptureRequest), StaticSystemDateTime.Past, "body");
