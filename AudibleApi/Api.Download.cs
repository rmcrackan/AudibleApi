...
public partial class Api
{
	...
	public async Task<string> WidevineDrmLicense(string asin, string licenseChallenge)
	{
		...
		if (drmLicenseDtoV10 is null)
		{
			var ex = new InvalidResponseException(
				response.RequestMessage?.RequestUri,
				responseJobj,
				$"DRM license response could not be deserialized for asin: [{asin}]");
			Serilog.Log.Logger.Error(ex, "DRM license response deserialization error");
			throw ex;
		}
	...
	public async Task<ContentLicense> GetDownloadLicenseAsync(string asin, DownloadQuality quality = DownloadQuality.High, ChapterTitlesType chapterTitlesType = ChapterTitlesType.Tree, DrmType drmType = DrmType.Adrm, bool spatial = false, params string[] additionalCodecs)
	{
		...
		catch (ApiErrorException ex)
		{
			...
			try
			{
				...
				if (contentLic.ContentMetadata is null)
					throw new ApplicationException("Could not retrieve content metadata for AAX workaround.");
		...
		try
		{
			if (_identityMaintainer.DeviceType is null)
				throw new InvalidOperationException("IdentityMaintainer DeviceType is not set in IdentityMaintainer.");
			if (_identityMaintainer.DeviceSerialNumber is null)
				throw new InvalidOperationException("IdentityMaintainer DeviceSerialNumber is not set in IdentityMaintainer.");
			if (_identityMaintainer.AmazonAccountId is null)
				throw new InvalidOperationException("IdentityMaintainer AmazonAccountId is not set in IdentityMaintainer.");
	...
	private async Task<List<string>> getAllCodecsAsync(string asin)
	{
		...
		do
		{
			...
			if (codecs?.Count is null or 0)
				throw new ApplicationException("Book has no available codecs. Cannot continue with download. Full book data:\r\n" + bookJson);
	...
	{
		var license = await GetDownloadLicenseAsync(asin);
		return license.PdfUrl ?? throw new ApplicationException("No PDF URL available for this book.");
	}
}
