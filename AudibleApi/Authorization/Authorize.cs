...
public class Authorize : IAuthorize
{
	...
	public async Task<AccessToken> ExtractAccessTokenAsync(HttpResponseMessage response)
	{
		...
		var accessTokenStr = authRegisterJson["access_token"]?.ToString() ?? throw new RegistrationException("No access_token in response");
		var expires = int.Parse(authRegisterJson["expires_in"]?.ToString() ?? throw new RegistrationException("No expires_in in response"));
